...
from Utilis.EventDectection.WAMMA import WAMMA
...
wamma = WAMMA(int(WAMMA_WINDOW_SEC * EVENT_SAMPLING_RATE),
            int(WAMMA_EDGE_SEC * EVENT_SAMPLING_RATE),
            WAMMA_P_THRE,  WAMMA_R_THRE)
...
for idx in range(data_len):
    ...
    if len(P_EVENT_BUFFER) == P_EVENT_BUFFER_LEN:
        event = wamma.update(np.mean(P_EVENT_BUFFER))
        winDuration = wamma.last_winSize / EVENT_SAMPLING_RATE
        P_EVENT_BUFFER = []
